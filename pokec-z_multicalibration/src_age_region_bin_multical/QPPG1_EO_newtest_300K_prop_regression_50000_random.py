...
def QPG_EO(data,adj_mtx, beta_avg, t):     #here we are taking two sensitive attrib # make the data here  
    ...
    X = pd.read_csv("../../../../UGE-Unbiased-Graph-Embedding/processed_data/pokec-z_node_attribute4.csv")

    for i in range(n_old):
        x = int(data_old.iloc[i,0])
        y = int(data_old.iloc[i,1])
        for j in range(2):
          for k in range(2):
...
for t in range(10):
  ...
  score,eo,dp = QPG_EO(data,adj_mtx,0.5,0.7)
