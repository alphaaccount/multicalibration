...
def QPG_EO(data,adj_mtx, beta_avg, t):     #here we are taking two sensitive attrib # make the data here  
    ...
    np.savetxt('y_true_pokec.txt', c_old)
    np.savetxt('y_pred_pokec.txt', c1_old)
    np.savetxt('y_prob_pokec.txt', b_old)
    ...
    write_lists_to_file('y_true_group.txt',actual)
    write_lists_to_file('y_prob_group.txt',scores)
    write_lists_to_file('y_pred_group.txt',convert_list_of_lists(scores))
    ...
    for alpha in Alpha:
      ...
      for indicator in indicator_all:
        ...
        np.savetxt('y_pred_pokec_fair'+str(alpha)+'.txt', fair)
        np.savetxt('y_prob_pokec_fair'+str(alpha)+'.txt', fair_scores)
        ...
        write_lists_to_file('y_prob_group_fair_'+str(alpha)+'.txt',scores)
        write_lists_to_file('y_pred_group_fair_'+str(alpha)+'.txt',convert_list_of_lists(scores))
