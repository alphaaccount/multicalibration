...
def QPG_EO(data,adj_mtx, beta_avg, t):     #here we are taking two sensitive attrib # make the data here  
    ...
    for base in base_all:
      ...
      for indicator in indicator_all:
        ...
        f = reg.predict(X2)
        print(f)
        np.savetxt("Test_regressed_scores", f)
        ...
        for i in range(n_old):
          if fair_scores[i] > 0.8:
            fair[i] = 1
          else:
            fair[i] = 0
        counter = 0
        for i in range(n_old):
          if i<=50000 and fair[i]==1:
           counter = counter + 1 
        print("Predicted edges in training set is", counter)
        counter = 0
        for i in range(n_old):
          if i>50000 and fair[i]==1:
           counter = counter + 1
        print("Predicted edges in test set is", counter)
        c = c_old
        b = b_old
        c1 = c1_old
        data = data_old
        n = len(data)
        print("classwise Precision, recall, f1scores of unfair scores are", precision_recall_fscore_support(c, c1))
        print("classwise Precision, recall, f1scores of fair scores are", precision_recall_fscore_support(c, fair))
