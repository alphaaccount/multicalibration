...
def QPG_EO(data,adj_mtx, beta_avg, t):     #here we are taking two sensitive attrib # make the data here  
    ...
    data = pd.read_csv('test_pairs_final2', sep=',')
    n = len(data)
    ...
    for i in range(len(data)):
        ...
        for j in range(2):
          for k in range(2):
            if int(X.iloc[x,0])==j and int(X.iloc[y,0])==k :
              data1[2*j+k][i] = 1
              a_adar_gr[2*j+k] =  a_adar_gr[2*j+k] + adamic_adar[i]
            if int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and sigmoid(np.inner(embedding_np[int(data.iloc[i,0])],embedding_np[int(data.iloc[i,1])])) > t:
            ...
            if int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and adj_mtx[x, y] > 0:
    ...
    for base in base_all:
      ...
      for indicator in indicator_all:
        ...
        constraints += [data1 @ cp.multiply(c2, x) <= beta*sizes_edges_actual + base*sizes_edges_actual]
        constraints += [data1 @ cp.multiply(c2, x) >= base*sizes_edges_actual]
        constraints += [x>=0, x<=1]
        ...
        for i in range(n):
              ...
              for j in range(2):
                for k in range(2):
                  ...
                  if(int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and c[i]==c1[i] and c[i]==1):
                  ...
                  if(int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and c[i]==0 and c1[i]==1):
                  ...
                  if(int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and c[i]==1 and c1[i]==0):
                    hit3[2*j+k] = hit3[2*j+k]+1
                    hit_sum3[2*j+k] = hit_sum3[2*j+k] + fair_scores[i]
                  if(int(X.iloc[x,0])==j and int(X.iloc[y,0])==k and c[i]==0 and c1[i]==0):
        ...
        for node_id in s:
          ...
          #neg_nodes = np.random.choice(np.where(node_edges == 0)[0], 100-num_pos, replace=False)
            ...
            all_eval_edges = np.zeros(1000)  # because each node has 20 neighbors in the set
