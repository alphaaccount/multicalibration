...
def QPG_EO(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
    ...
    graph_embedding = pk.load(open("../../../UGE-Unbiased-Graph-Embedding/embeddings/twitter_gat_entropy_0.01_800_embedding.bin",'rb'))
...
#     report_index(index,data1,e):  
    ...
    for alpha in Alpha:
      ...
      for indicator in indicator_all:
        ...
        write_lists_to_file('y_true_twitter_group.txt',actual)
        write_lists_to_file('y_prob_twitter_group_fair_'+str(alpha)+'.txt',scores)
        write_lists_to_file('y_pred_twitter_group_fair_'+str(alpha)+'.txt',convert_list_of_lists(scores))
