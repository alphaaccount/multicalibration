...
def find_dp_ndcg_all(data, adj_mtx):
    ...
    graph_embedding = pk.load(open('../../../DeBayes/cne_dump/cora_seed_0_dim_8_trp_bias-deg.emb', 'rb'))
    ...
    train_array = np.loadtxt('../../../multicalibration_link_prediction/train_cora.txt')
    test_array = np.loadtxt('../../../multicalibration_link_prediction/test_cora.txt')
    val_array = np.loadtxt('../../../multicalibration_link_prediction/val_cora.txt')

    predictions = np.loadtxt('../../../multicalibration_link_prediction/HKRR_cora.txt', delimiter=',')
    ...
    for i in range(len(data)):
      x = int(data.iloc[i,0])
      y = int(data.iloc[i,2])
      s1 = predictions[hkrr_dict[x]][1]
      s2 = predictions[hkrr_dict[y]][1]
      z = abs(s1 - s2)
      if(z < 0.1):
        b_old.append(0.9)
      else:
      ...
      for j in range(6):
        for k in range(6):
          if int(X.iloc[x,2])==j and int(X.iloc[y,2])==k :
            brier[6*j+k+4] = brier[6*j+k+4] + (b[i]-adj_mtx[x,y])**2
            scores[6*j+k+4].append(b[i])
            actual[6*j+k+4].append(adj_mtx[x,y])
      '''
    for i in range(m):
      if sizes[i] != 0:
    ...
    print("Micro F1 score for unfair score (binarised) is", f1_score(c,b, average='micro'))
    print("Micro F1 score for fair score (binarised) is", f1_score(c,fair, average='micro'))
    print("Macro F1 score for unfair score (binarised) is", f1_score(c,b, average='macro'))
    print("Macro F1 score for fair score (binarised) is", f1_score(c,fair, average='macro'))
    print("Binary F1 score for unfair score (binarised) is", f1_score(c,b, average='binary'))
    print("Binary F1 score for fair score (binarised) is", f1_score(c,fair, average='binary'))
    print("Weighted F1 score for unfair score (binarised) is", f1_score(c,b, average='weighted'))
    print("Weighted F1 score for fair score (binarised) is", f1_score(c,fair, average='weighted'))
    print("Accuracy score for unfair score (binarised) is", accuracy_score(c,b))
    ...
    print("Micro Precision score for unfair score (binarised) is", precision_score(c,b, average='micro'))
    print("Mirco Precision score for fair score (binarised) is", precision_score(c,fair, average='micro'))
    print("Mirco Recall score for unfair score (binarised) is", recall_score(c,b, average='micro'))
    ...
    print("Binary Precision score for unfair score (binarised) is", precision_score(c,b, average='binary'))
