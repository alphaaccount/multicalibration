...
def find_dp_ndcg_all(data, adj_mtx):
    ...
    graph_embedding = np.load("../../../FairEGM/results/cora/embeddings/gfo_d-32_d2-16_i-glorot_normal_i2-glorot_normal_19.npy")
          ...
          for j in range(7):
            for k in range(7):
              if int(data.iloc[i,1])==j and int(data.iloc[i,3])==k :
                ...
                grf_eo[7*j+k].append(sigmoid(fair_scores[i])*c[i])
