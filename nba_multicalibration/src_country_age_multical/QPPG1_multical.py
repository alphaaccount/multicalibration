...
def QPG_EO(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
    ...
    graph_embedding = pk.load(open("../../../../UGE-Unbiased-Graph-Embedding/embeddings/nba_gat_entropy_0.01_800_embedding.bin",'rb'))
...
#     report_index(index,data1,e):  
    ...
    for i in range(len(data)):
        for j in range(2):
          for k in range(2):
            ...
            if data.iloc[i,1]==j and data.iloc[i,4]==k and sigmoid(data.iloc[i,6]) > 0.5:
        ...
        for j in range(11):
          for k in range(11):
            ...
            if data.iloc[i,2]==j+1 and data.iloc[i,5]==k+1 and adj_mtx[int(data.iloc[i,0]),int(data.iloc[i,3])] > 0:
              sizes_edges_actual[11*j+k+4] = sizes_edges_actual[11*j+k+4]+1
            if data.iloc[i,1]==j+1 and data.iloc[i,3]==k+1 and sigmoid(data.iloc[i,6]) > 0.5:
    ...
    indicator_all = [0]
    Alpha = [0.001, 0.005, 0.01] #[0.2, 0.4, 0.5, 0.001, 0.005, 0.01, 0.05, 0.1, 
    for alpha in Alpha:
      ...
      for indicator in indicator_all:
        ...
        for i in range(len(data)):
            ...
            for j in range(11):
              for k in range(11):
                if data.iloc[i,2]==j+1 and data.iloc[i,5]==k+1 :
                  data_new[11*j+k+4][i] = 1   #begins at 4
        ...
        write_lists_to_file('y_true_nba_group.txt',actual)
        write_lists_to_file('y_prob_nba_group_fair_'+str(alpha)+'.txt',scores)
        write_lists_to_file('y_pred_nba_group_fair_'+str(alpha)+'.txt',convert_list_of_lists(scores))
