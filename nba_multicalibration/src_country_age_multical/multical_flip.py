...
def find_dp_ndcg_scores(data, adj_mtx):
    m = 121
    n = len(data)
    data1 = np.zeros((m,n))
    
    #graph_embedding = np.load("../../../../FairEGM/results/nba/embeddings/augmented_d-32_d2-16_i-glorot_normal_i2-glorot_normal_18.npy")
    graph_embedding = np.genfromtxt('../../../../FLIP_master/FLIP/flip_nba_age.scores', delimiter=' ')
    ...
    for i in range(n):
        fair_scores.append(embedding_np[i,0])
        for j in range(11):
          for k in range(11):
            if int(data.iloc[i,2]) == j+1 and int(data.iloc[i,5]) == k+1:
              gr[11*j+k].append(data.iloc[i,6])
              grf[11*j+k].append(fair_scores[i])
              data1[11*j+k][i] = 1
            if int(data.iloc[i,2]) == j+1 and int(data.iloc[i,5]) == k+1 and adj_mtx[int(data.iloc[i,0]),int(data.iloc[i,3])]>0:
    ...
    for i in range(n):
        adj_mtx_unfair[int(data.iloc[i,0])][int(data.iloc[i,2])] = sigmoid(data.iloc[i,6])
...
def find_dp_ndcg(data, adj_mtx):
    ...
    #graph_embedding = np.load("../../../../FairEGM/results/nba/embeddings/augmented_d-32_d2-16_i-glorot_normal_i2-glorot_normal_18.npy")
    graph_embedding = np.genfromtxt('../../../../FLIP_master/FLIP/flip_nba_age.embedding', delimiter=' ')
    embedding_df = pd.DataFrame(graph_embedding)
    #embedding_df = embedding_df.rename(index=int, columns={0:"user_id"})
    #print(embedding_df)
    #user_ids = embedding_df['user_id']
...
def find_dp_ndcg_all(data, adj_mtx):
    ...
    graph_embedding = np.genfromtxt('../../../../FLIP_master/FLIP/flip_nba_age.embedding', delimiter=' ')  #np.load("../../../FairEGM/results/nba/embeddings/gfo_d-32_d2-16_i-glorot_normal_i2-glorot_normal_19.npy")
    embedding_df = pd.DataFrame(graph_embedding)
    #embedding_df = embedding_df.rename(index=int, columns={0:"user_id"})
    #print(embedding_df)
    #user_ids = embedding_df['user_id']
    ...
    for i in range(n):
        ...
        for j in range(2):
          for k in range(2):
            if int(data.iloc[i,1]) == j and int(data.iloc[i,4]) == k:
              gr[2*j+k].append(data.iloc[i,6])
              grf[2*j+k].append(fair_scores[i])
              data1[2*j+k][i] = 1
            if int(data.iloc[i,1]) == j and int(data.iloc[i,5]) == k and adj_mtx[int(data.iloc[i,0]),int(data.iloc[i,3])]>0:
              sizes_edges[2*j+k] = sizes_edges[2*j+k] + 1

        for j in range(11):
          for k in range(11):
            if int(data.iloc[i,2]) == j+1 and int(data.iloc[i,5]) == k+1:
              gr[11*j+k+4].append(data.iloc[i,6])
    ...
    print("Max smooth calibration error is", smcalib_error.max()-np.min(smcalib_error[smcalib_error>0]))
...
print('loading data ...')
with open('../../../../UGE-Unbiased-Graph-Embedding/processed_data/nba_edge.csv', 'r') as fin:
    lines = fin.readlines()
    idx = 0 
    for line in lines:
        if idx == 0: 
