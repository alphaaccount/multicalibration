...
def QPG_EO(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
    m = 121 # total 40 groups are there 
    n = len(data)
    data1 = np.zeros((m,n))
    ...
    for i in range(len(data)):
        ...
        for j in range(11):
          for k in range(11):
            ...
            if data.iloc[i,2]==j+1 and data.iloc[i,5]==k+1 and sigmoid(data.iloc[i,6]) > t:
...
def QPG_EO_all(data,adj_mtx, beta_avg):     #here we are taking two sensitive attrib # make the data here  
